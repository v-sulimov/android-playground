 ...
 * and updates the [ScreenState] accordingly.
 */
class EventsReducerTest {

    /**
     * Tests that the [EventsReducer] updates the events list in [ScreenState.EventsList]
    ...
    fun `reduceTyped updates events list when state is EventsList and action is UpdateEventsList`() {
        ...
        val state = ScreenState.EventsList(events = initialEvents)
        ...
        assertTrue(newState is ScreenState.EventsList)
        val eventsListState = newState
        assertEquals(newEvents, eventsListState.events)
    }

    /**
     * Tests that the [EventsReducer] returns the current [ScreenState] unchanged
     * when the state is not [ScreenState.EventsList] and the action is [EventsAction.UpdateEventsList].
     */
    @Test
    fun `reduceTyped returns same state when state is not EventsList and action is UpdateEventsList`() {
        val state = ScreenState.Onboarding()
        val newEvents = listOf(
            EventState(details = "Event 3", timestamp = 3L, formattedTimestamp = "Today"),
            EventState(details = "Event 4", timestamp = 4L, formattedTimestamp = "Today")
        )
