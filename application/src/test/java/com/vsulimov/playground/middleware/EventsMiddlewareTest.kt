...
class EventsMiddlewareTest {
    ...
    fun `SaveEvent action saves to SharedPreferences and updates events list when state has events`() {
        val state = ApplicationStateTestFactory.createApplicationState(screenState = ScreenState.EventsList())
        ...
        assertEquals(
            expected = 1, actual = updateAction.newEvents.size, message = "Events list should contain one event"
        )
    ...
    fun `SaveEvent action saves to SharedPreferences but does not call next when state has no events`() {
        val state = ApplicationStateTestFactory.createApplicationState(screenState = ScreenState.Onboarding())
        val action = EventsAction.SaveEvent("Test Event")
    ...
    fun `DeleteEvent action removes from SharedPreferences and updates events list when state has events`() {
        ...
        val screenState = ScreenState.EventsList(events = events)
        val state = ApplicationStateTestFactory.createApplicationState(screenState = screenState)
