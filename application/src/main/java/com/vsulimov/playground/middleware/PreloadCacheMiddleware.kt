...
import com.vsulimov.playground.state.ScreenState.EventsList
...
class PreloadCacheMiddleware(
     ...
     */
    private fun preloadCachedEventsList(originalAction: NavigateTo, next: (Action) -> Unit) {
        val savedEvents = sharedPreferences.all.toSortedMap().map {
            EventState(
                timestamp = it.key.toLong(),
                details = it.value.toString(),
                formattedTimestamp = UnixTimestampConverter.timestampToHumanReadable(
                    timestamp = it.key.toLong(),
